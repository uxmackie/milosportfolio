(() => {
  ...
  if(!ctx || layers.some(layer=>!layer.ctx)){button.hidden=true;return;}
  ...
  function label(){button.querySelector('.motion-label').textContent=paused?'Play motion':'Pause motion';button.querySelector('.motion-icon').textContent=paused?'▷':'Ⅱ';button.setAttribute('aria-label',paused?'Play background animation':'Pause background animation');button.setAttribute('aria-pressed',String(paused));}
  button.addEventListener('click',()=>{paused=!paused;last=0;label();refresh();});
  reduced.addEventListener('change',()=>{paused=reduced.matches;last=0;label();refresh();});
  ...
  picture.onload=()=>{loaded=true;measure();label();};
  picture.onerror=()=>{background.hidden=true;surfaces.forEach(canvas=>canvas.hidden=true);button.hidden=true;};
  picture.src='assets/background.jpg';label();
})();
