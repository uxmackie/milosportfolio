(() => {
  ...
  let active = 0,
    ...
    previous = 0;
  function spring(now) {
    const dt = Math.min((now - (previous || now)) / 1000, 0.032);
    previous = now;
    const w = 30,
      d = x - target,
      c = velocity + w * d,
    ...
    if (Math.abs(x - target) > 0.05 || Math.abs(velocity) > 0.05)
      springFrame = requestAnimationFrame(spring);
    else {
      selection.style.transform = `translateX(${target}px)`;
      x = target;
      springFrame = 0;
      previous = 0;
    }
  ...
  function fitPanel(instant = false) {
    ...
    const panel = document.getElementById(
      tabs[active].getAttribute("aria-controls"),
    );
    const next = panel.getBoundingClientRect().height;
    if (Math.abs(next - current) < 0.5 && !sizeAnimation) return;
    if (sizeAnimation) {
      sizeAnimation.cancel();
      sizeAnimation = null;
    }
    area.style.height = `${next}px`;
    if (!instant && !reduced.matches && Math.abs(current - next) > 0.5) {
      const animation = area.animate(
        [{ height: `${current}px` }, { height: `${next}px` }],
        { duration: 260, easing: "cubic-bezier(.23,1,.32,1)" },
      );
      sizeAnimation = animation;
      animation.onfinish = () => {
        if (sizeAnimation === animation) sizeAnimation = null;
      };
    }
  }
  ...
  function select(index, keyboard = false) {
    ...
    for (const [i, tab] of tabs.entries()) {
      ...
      if (selected && !keyboard && !reduced.matches)
        panel.animate(
          [
    ...
    window.dispatchEvent(
      new CustomEvent("portfolio:tab", { detail: tabs[index].dataset.tab }),
    );
    position(keyboard);
    if (keyboard) tabs[index].focus();
  }
