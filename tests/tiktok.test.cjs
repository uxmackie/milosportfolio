...
test('TikTok endpoint: auth, scoped data, cache, partial failure, expired token',async()=>{
 const oldFetch=global.fetch,oldToken=process.env.TIKTOK_ACCESS_TOKEN;
 try{
  delete process.env.TIKTOK_ACCESS_TOKEN;
  assert.equal((await call(handler())).body.status,'not_connected');
  process.env.TIKTOK_ACCESS_TOKEN='secret-test-token';let calls=0;
 ...
 }finally{global.fetch=oldFetch;if(oldToken===undefined)delete process.env.TIKTOK_ACCESS_TOKEN;else process.env.TIKTOK_ACCESS_TOKEN=oldToken;}
});
